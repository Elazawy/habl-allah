...
import {
  fetchTeacherAdminById,
  fetchTeacherReviews,
  createTeacherReview,
  updateTeacherReview,
  deleteTeacherReview,
  uploadTeacherReviewImage,
  deleteReviewImage,
...
export default function TeacherReviewsManagementPage() {
  const { id } = useParams();
  ...
  const togglePublished = async (review) => {
    try {
      const updated = await updateTeacherReview(review.id, { is_published: !review.is_published });
      handleSaved(updated);
    } catch (error) {
      console.error(error);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const removed = await deleteTeacherReview(deleteTarget.id);
      setReviews((prev) => prev.filter((r) => r.id !== deleteTarget.id));
      setDeleteTarget(null);

      if (removed?.image_path) {
        deleteReviewImage(removed.image_path).catch((cleanupError) => {
          console.error('[cleanup teacher review image failed]', cleanupError);
        });
      }
    } catch (error) {
      console.error(error);
    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div>
          ...
          <h1 className="admin-page-title">مراجعات المعلم</h1>
          <p className="admin-page-desc">
            {teacher ? `${teacher.name} — ${reviews.length} مراجعة` : 'إدارة صور آراء الطلاب الخاصة بالمعلم'}
          </p>
        </div>
        <button
          id="admin-add-teacher-review-btn"
      ...
      <div className="admin-filters">
        <div className="admin-search-wrapper">
          ...
          <input
            id="admin-teacher-reviews-search"
            type="text"
      ...
      {loading ? (
        ...
        <div className="admin-empty">
          <ImageIcon size={48} />
          <p>لم يتم العثور على هذا المعلم.</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="admin-empty">
          <ImageIcon size={48} />
          <p>
            {reviews.length === 0
              ? 'لا توجد مراجعات لهذا المعلم حتى الآن'
      ...
      {modalState.open && teacher && (
        <ReviewImageFormModal
          review={modalState.review}
          onClose={closeModal}
          onSaved={handleSaved}
          onUploadImage={(file) => uploadTeacherReviewImage(file, teacher.id)}
          onDeleteImage={deleteReviewImage}
          onCreate={createTeacherReview}
          onUpdate={updateTeacherReview}
          extraPayload={{ teacher_id: teacher.id }}
          titleAdd="إضافة مراجعة للمعلم"
          titleEdit="تعديل مراجعة المعلم"
        />
      )}
